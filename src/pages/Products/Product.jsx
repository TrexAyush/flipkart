    ...
    return (
        <div className="flex flex-col items-start gap-2 px-4 py-6 relative hover:shadow-lg rounded-sm">
            ...
            <Link to={`/product/${_id}`} className="flex flex-col items-center text-center group">
                ...
                <h2 className="text-sm mt-4 group-hover:text-blue-600 text-left">{name.length > 85 ? `${name.substring(0, 85)}...` : name}</h2>
            </Link>
            {/* <!-- image & product title --> */}
            ...
            <div className="flex flex-col gap-2 items-start">
                ...
                <span className="text-sm text-gray-500 font-medium flex gap-2 items-center">
                    <span className="text-xs px-1.5 py-0.5 bg-green-600 rounded-sm text-white flex items-center gap-0.5">{ratings.toFixed(1)} <StarIcon sx={{ fontSize: "14px" }} /></span>
